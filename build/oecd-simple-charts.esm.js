...
import { sum, max, range, extent } from 'd3-array';
...
var PearlChart = function (_OECDChart) {
  ...
  createClass(PearlChart, [{
    ...
    value: function render() {
      ...
      this.scale = scaleLinear().domain(extent$$1).range([0, innerWidth]);
      ...
      this.getAxis({ chartWrapper: this.chartWrapper, extent: extent$$1, innerWidth: innerWidth, innerHeight: innerHeight, labelOffset: labelOffset });
    ...
    value: function getAxis() {
      ...
      // render axis labels
      var axisLabel = axis.selectAll('.pearlchart__axis-label').data(extent$$1);
...
var StackedChart = function (_OECDChart) {
  ...
  createClass(StackedChart, [{
    ...
    value: function render() {
      ...
      this.x = scaleLinear().range([0, innerWidth]).domain(extent$$1);
...
var BoxPlot = function (_OECDChart) {
  ...
  createClass(BoxPlot, [{
    ...
    value: function getChart(data) {
      ...
      this.x = scaleLinear().range([0, innerWidth - innerMarginRight - innerMarginLeft]).domain(extent$$1);
      ...
      var ticks = (extent$$1[1] - extent$$1[0]) / step;
...
function getColorRange(base, amount, lightness) {
  return range(0, amount, 1).map(function (step) {
...
var RadialBarChart = function (_OECDChart) {
  ...
  createClass(RadialBarChart, [{
    ...
    value: function render() {
      ...
      var arcGenerator = arc().innerRadius(function (d, i) {
      ...
      var colorData = rows.map(function (row, i) {
        ...
        var extent$$1 = extent(data, function (d) {
          return +d[row];
        });
        return scaleQuantize().domain(extent$$1).range(colors.slice(0).reverse());
      });
      ...
      arcGroups.append('g').classed('arc-container', true).selectAll('.arc').data(function (d, i) {
        ...
        select(this).attr('stroke-width', hoverStrokeWidth).attr('stroke', hoverStrokeColor);

        that.event.emit('mouseenter', d);
      }).on('mouseleave', function (d) {
        select(this).attr('stroke-width', 1).attr('stroke', strokeColor);
      ...
      var longestLabel = max(svgRowLabels.nodes(), function (label) {
