...
var StackedChart = function (_OECDChart) {
  ...
  createClass(StackedChart, [{
    ...
    value: function render() {
      ...
      this.x = linear$1().range([0, innerWidth]).domain(extent);
...
var RadialBarChart = function (_OECDChart) {
  ...
  createClass(RadialBarChart, [{
    ...
    value: function render() {
      ...
      var lightenFactor = 2 / colorSteps;

      var colorData = rows.map(function (row, i) {
        var baseColor = hsl(rowColors[i]);
        var colors = colorRange.map(function (step$$1, i) {
          return baseColor.brighter(i * lightenFactor).hex();
        });
        var extent$$1 = d3Extent(data, function (d) {
          return +d[row];
        });
        return threshold$1().domain(extent$$1).range(colors);
      });
    ...
    value: function handleGroupMouseLeave() {
